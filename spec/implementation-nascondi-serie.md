...
- **2026-09-22 — il filtro delle nascoste diventa esclusivo, su richiesta dell'utente a feature già chiusa.** «Mostra nascoste» aggiungeva le nascoste alle altre; ora «Mostra serie nascoste» mostra **solo** le nascoste e «Mostra serie attive» **solo** le attive, incrociate come prima con la lente e con «Mostra completate». Costo: un predicato in `useTrackedShows.ts` (`listedEntries` passa da `showHidden || !isHiddenShow` a `isHiddenShow === showHidden`), le due etichette in `HiddenShowsToggle.vue`, il messaggio del dettaglio che le citava, e una `computed` nuova in `HomeView.vue`. Quest'ultima non era nella richiesta ed è la parte che conta: con un filtro esclusivo e la preferenza persistita in `localStorage`, il vecchio `v-if="hiddenCount > 0"` faceva sparire il pulsante appena la lente restava senza nascoste, lasciando una lista vuota e **nessun modo di tornare alle attive**. La condizione è ora `hiddenCount > 0 || showHidden`, con un test che muore se si ripristina la vecchia. Aggiornati SPEC §§ 3, 4, 5, 10, 11 e criteri 5-8. Suite a **637**.
