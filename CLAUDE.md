...
PWA mobile-first per tracciare le serie TV seguite da Fabio e Irene: una sola lista condivisa, per sapere quali serie stanno seguendo, qual è la prima puntata ancora da vedere, quante puntate già uscite restano arretrate e quando arrivano le nuove. **Uso privato, due persone.**
...
Documenti di riferimento: `spec/tv-tracker-spec.md` è la fonte autorevole sul comportamento atteso; `spec/implementation-tv-tracker.md` contiene piano, decisioni prese e il registro fase per fase, comprese le correzioni trovate durante la verifica di ogni fase.
...
Il costo reale di questo confine, misurato: attivare Firestore (Fase 22 del piano) è costato **due file nuovi dentro `src/persistence/` più una riga** in `currentTrackedShowStore.ts` — nessuna vista, nessun composable.
...
Il numero esatto di test va letto dall'ultima esecuzione di `npm test`: è cresciuto nel corso dello sviluppo (394 alla chiusura della Fase 23) e non è opportuno fissarlo come cifra statica in un documento che invecchia col codice.
...
Fasi 1–23 del piano chiuse: MVP locale completo (Fasi 1–19), Firebase Authentication e Cloud Firestore attivi come implementazioni del confine dati (Fasi 20–22), regole di sicurezza Firestore pubblicate e travaso dei dati locali eseguito (Fase 23). L'app è pubblicata e in uso su [tv-tracker-ds1.pages.dev](https://tv-tracker-ds1.pages.dev).
