...
### Fase 25 — Le serie completate spariscono dalla lista (fuori piano originale)

*Obiettivo: la home mostra solo ciò che resta da guardare.*

- [x] `WatchPosition.isCompleted`: nessun episodio non visto, né pubblicato né annunciato. Distinto da `isCaughtUp`, che è vero anche con una puntata futura in arrivo.
- [x] Preferenza mostra/nascondi in `localStorage`, default «nascondi», stessa meccanica di `useSortPreference`.
- [x] `useTrackedShows` filtra le completate ed espone `completedCount`, contato sempre su tutte le serie.
- [x] Pulsante con conteggio accanto all'ordinamento; controlli visibili finché esiste almeno una serie seguita.
- [x] `summaryText`, `trackedProviderShowIds` e le righe con dati non allineati non risentono del filtro.
- **File letti:** `tv-tracker-spec.md`, `useSortPreference.ts`, `SortSelect.vue`, `HomeView.vue`, `watchPosition.ts`.
- **File modificati:** `src/domain/watchPosition.ts` (+spec), `src/domain/showSorting.spec.ts`, `src/composables/useTrackedShows.ts` (+spec), `src/views/HomeView.vue` (+spec).
- **File creati:** `src/composables/useCompletedVisibilityPreference.ts` (+spec) — accanto a `useSortPreference.ts`, stesso concetto di preferenza locale; `src/components/show/CompletedVisibilityToggle.vue` (+spec) — accanto a `SortSelect.vue`, stesso concetto di controllo della lista. Nessuna cartella nuova.
...
| 2026-09-20 | 25 — Serie completate nascoste dalla lista | `npm test` (417 test), `lint` mirato, `typecheck` | **Fatto.** `isCompleted` su `WatchPosition` come stato **derivato** e mai persistito: una serie completata torna in lista da sé appena il catalogo annuncia una puntata. Preferenza in `localStorage` (`tv-tracker:show-completed`), default nascoste. Pulsante con conteggio accanto all_ordinamento; i controlli restano visibili anche quando il filtro nasconde tutte le serie, altrimenti la scelta sarebbe irreversibile. Sommario, blocco dei duplicati e righe con dati non allineati indifferenti al filtro. | **Eseguita, due correzioni.** (1) `hasAnyTrackedShow` leggeva la cardinalità di `trackedProviderShowIds`, insieme nato per il blocco dei duplicati: due concetti legati per caso, sostituiti da un `hasTrackedShows` dedicato. (2) Densità: `String(showCompleted)` annidato dentro `setItem`, estratto in variabile. *Confermata per rottura* la copertura dei due confini che contano: la distinzione `isCompleted`/`isCaughtUp` e la visibilità dei controlli a filtro totale. `solid-srp-reviewer` non invocato: si applica al solo Java. |
